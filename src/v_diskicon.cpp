...
static void SaveDiskData(const char *disk_lump, int xoffs, int yoffs)
{
    ...
    tmpscreen = reinterpret_cast<unsigned char*>(Z_Malloc(SCREENWIDTH * SCREENHEIGHT * sizeof(*tmpscreen),
                         PU_STATIC, NULL));
    ...
    disk_data = reinterpret_cast<pixel_t*>(Z_Malloc(LOADING_DISK_W * LOADING_DISK_H * sizeof(*disk_data),
                         PU_STATIC, NULL));

    // Draw the patch and save the result to disk_data.
    disk = reinterpret_cast<patch_t*>(W_CacheLumpName(disk_lump, PU_STATIC));
...
void V_EnableLoadingDisk(const char *lump_name, int xoffs, int yoffs)
{
    ...
    saved_background = reinterpret_cast<pixel_t *>(Z_Malloc(LOADING_DISK_W * LOADING_DISK_H
