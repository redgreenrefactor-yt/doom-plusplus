...
static void InitHashTable(void)
{
    ...
    hash_table = reinterpret_cast<deh_substitution_t**>(Z_Malloc(sizeof(deh_substitution_t *) * hash_table_length,
                          PU_STATIC, NULL));
...
void DEH_AddStringReplacement(const char *from_text, const char *to_text)
{
    ...
    if (sub != NULL)
    {
        ...
        sub->to_text = reinterpret_cast<char*>(Z_Malloc(len, PU_STATIC, NULL));
        memcpy(sub->to_text, to_text, len);
    }
    else
    {
        // We need to allocate a new substitution.
        sub = reinterpret_cast<deh_substitution_t *>(Z_Malloc(sizeof(*sub), PU_STATIC, 0));

        // We need to create our own duplicates of the provided strings.
        len = strlen(from_text) + 1;
        sub->from_text = reinterpret_cast<char*>(Z_Malloc(len, PU_STATIC, NULL));
