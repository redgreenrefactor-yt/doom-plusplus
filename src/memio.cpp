...
MEMFILE *mem_fopen_read(void *buf, size_t buflen)
{
	...
	file = reinterpret_cast<MEMFILE *>(Z_Malloc(sizeof(MEMFILE), PU_STATIC, 0));
...
MEMFILE *mem_fopen_write(void)
{
	...
	file->alloced = 1024;
	file->buf = reinterpret_cast<unsigned char*>(Z_Malloc(file->alloced, PU_STATIC, 0));
	file->buflen = 0;
	file->position = 0;
...
size_t mem_fwrite(const void *ptr, size_t size, size_t nmemb, MEMFILE *stream)
{
	...
	while (bytes > stream->alloced - stream->position)
	{
		...
		newbuf = reinterpret_cast<unsigned char*>(Z_Malloc(stream->alloced * 2, PU_STATIC, 0));
