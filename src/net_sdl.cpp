...
static void NET_SDL_InitAddrTable(void)
{
    ...
    addr_table = reinterpret_cast<addrpair_t **>(Z_Malloc(sizeof(addrpair_t *) * addr_table_size,
                          PU_STATIC, 0));
...
static net_addr_t *NET_SDL_FindAddress(IPaddress *addr)
{
    ...
    if (empty_entry < 0)
    {
        ...
        new_addr_table = reinterpret_cast<addrpair_t **>(Z_Malloc(sizeof(addrpair_t *) * new_addr_table_size,
                                  PU_STATIC, 0));
    ...
    new_entry = reinterpret_cast<addrpair_t *>(Z_Malloc(sizeof(addrpair_t), PU_STATIC, 0));
...
net_addr_t *NET_SDL_ResolveAddress(const char *address)
{
    ...
    colon = const_cast<char*>(strchr(address, ':'));
