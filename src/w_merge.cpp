...
static void InitSpriteList(void)
{
    if (sprite_frames == NULL)
    {
        ...
        sprite_frames = reinterpret_cast<sprite_frame_t *>(Z_Malloc(sizeof(*sprite_frames) * sprite_frames_alloced,
                                 PU_STATIC, NULL));
    }
...
static sprite_frame_t *FindSpriteFrame(char *name, int frame)
{
    ...
    if (num_sprite_frames >= sprite_frames_alloced)
    {
        ...
        newframes = reinterpret_cast<sprite_frame_t *>(Z_Malloc(sprite_frames_alloced * 2 * sizeof(*sprite_frames),
                             PU_STATIC, NULL));
